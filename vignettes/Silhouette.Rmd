...
To use the Probability of Alternative Cluster (PAC) method (which is more penalised variation of mediod method) and return a sorted output:
...
The `Silhouette` function prints overall and cluster-wise silhouette indices to the R console by default, but these values are not directly stored in the returned object. To extract them programmatically, use the `summary()` function:
...
```
  ...
  The optimal number of clusters is often suggested by the "elbow" or "reverse elbow poit"—the point after which increases in *k* lead to diminishing improvements in silhouette width. This visual guide is valuable for assessing the clustering structure in your data.
...
```{r plot4, fig.width=6, fig.height=4, fig.alt = "fig4.4"}
  ...
  sil_fuzzy <- Silhouette(prox_matrix = "d", prob_matrix = "u", clust_fun = fcm,
...
- **Step 2: Compute Silhouette Widths for Each Mode**  
  For each dimension (e.g., rows and columns), calculate silhouette widths using the membership probability matrices (`result@rowposteriorprob` for rows, `result@colposteriorprob` for columns) via the `softSilhouette()` function:
