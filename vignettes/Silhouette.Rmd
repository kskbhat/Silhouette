...
**Output**: The `summary()` call displays a table of cluster sizes and average silhouette widths, with the overall average silhouette width indicating clustering quality. The `plot()` creates a bar plot where each bar represents an observation’s silhouette width, colored by cluster, with a dashed line showing the average silhouette width.

## 2. 🧠 Fuzzy Silhouette with FCM Clustering

For soft clustering, `softSilhouette()` uses membership probabilities (e.g., from `ppclust::fcm`) to compute fuzzy silhouette widths.

```{r fuzzy-silhouette, fig.width=6, fig.height=4, fig.alt = "fig2"}
  library(ppclust)
  data(iris)
...
```{r scree-plot, fig.width=6, fig.height=4,, fig.alt = "fig3"}
  library(ppclust)
  library(ggplot2)
  data(iris)
...
## 5. 🔬 Extended Silhouette for Multi-Way Clustering
