...
This example demonstrates how to compute silhouette widths for a clustering result when the proximity (distance) matrix between observations and cluster centers is unknown.
...
The integrated use of `Silhouette()`, `softSilhouette()`, `cerSilhouette()`, `dbSilhouette()`, `clust_fun`, and summary functions makes this analysis straightforward and efficient for both crisp and fuzzy clustering frameworks.
...
```{r plot3.1, fig.width=7, fig.height=6, fig.alt = "fig4.3.1", eval = requireNamespace("drclust", quietly = TRUE)}
library(drclust)
...
The `calSilhouette()` function provides a streamlined approach to compute and compare all available silhouette methods from the package in a single call. This function is particularly useful for:
...
**Key Features:**
- Automatically computes all compatible silhouette methods based on available input matrices
- Returns a comprehensive summary data frame comparing crisp, fuzzy, and median silhouette values
- Supports both direct matrix input and clustering function output
- Computes up to 11 different silhouette methods when both proximity and probability matrices are provided
...
- `pp_pac`, `pp_medoid` - Posterior probabilities with PAC/Medoid methods
- `nlpp_pac`, `nlpp_medoid` - Negative log posterior probabilities with PAC/Medoid methods
- `pd_pac`, `pd_medoid` - Probability distribution with PAC/Medoid methods
- `cer` - Certainty-based silhouette
- `db` - Density-based silhouette

### a. Comprehensive Method Comparison Using Clustering Function

This example demonstrates how to use `calSilhouette()` with a clustering function to automatically compute all available silhouette methods:
...
A powerful application of `calSilhouette()` is comparing multiple clustering algorithms across all silhouette methods:
...
```{r calSil4, fig.width=8, fig.height=5, fig.alt = "fig7.1"}
library(ggplot2)
library(tidyr)
...
Use `calSilhouette()` to evaluate clustering quality across different numbers of clusters:

```{r calSil5, fig.width=8, fig.height=5, fig.alt = "fig7.2"}
...
When only the proximity matrix is available (e.g., for crisp clustering), `calSilhouette()` automatically computes only the applicable methods:
...
```{r calSil8, fig.width=8, fig.height=6, fig.alt = "fig7.3"}
library(ggplot2)
library(tidyr)
...
- Use `calSilhouette()` when you need a comprehensive overview of clustering quality across multiple methodological perspectives
- The function is especially useful for algorithm comparison studies and sensitivity analyses
...
- The `a` parameter controls the fuzzifier for weighted averaging in fuzzy methods (default = 2)
- Consider using heatmaps or grouped bar plots to visualize method comparisons effectively
...
-   **Step 1: Generate Mock Two-Way Membership Probability Matrices**\
    Suppose we have partitioned a dataset along two modes (e.g., rows and columns of a matrix) using a biclustering algorithm. Let the row clustering result in membership probabilities for 150 observations across 3 clusters, and the column clustering result in probabilities for 4 variables across 2 clusters.
...
-   **Step 2: Compute Silhouette Widths for Each Mode**\
    For each dimension (e.g., rows and columns), calculate silhouette widths using the membership probability matrices via the `softSilhouette()` function:
...
The table below contrasts the `Silhouette` package with the closest CRAN alternatives, focusing on key algorithmic, statistical, and software features. While the classical silhouette is implemented by the `cluster` package (with visualization extensions in `factoextra`), the `Silhouette` package is unique in providing a unified framework for soft/fuzzy, density-based, certainty, and multi-way silhouettes with highly scalable $O(nK)$ algorithms.
...
* **Input Proximity Representation**: The `Silhouette` package consumes an $n \times K$ proximity matrix rather than the $n \times n$ `dist` object used by `cluster::silhouette()`. This reduces memory overhead by a factor of $n / K$.
* **Soft Partition Support**: Unlike `cluster` and `factoextra` (which only support hard partitions), and `fclust` (which does not natively expose soft silhouette variants), our package supports four distinct soft silhouette variants.
* **Unified Plotting**: Every producer function returns a customizable `ggplot2` object.

# Computational Considerations & Benchmarks

The asymptotic complexity of the `Silhouette` package follows from its $n \times K$ representation. Every computation scales linearly at **$O(nK)$** in both time and memory. In contrast, the classical Rousseeuw silhouette requires calculating and storing the full $n \times n$ dissimilarity matrix, scaling quadratically at **$O(n^2)$**.

To evaluate these scaling claims empirically, we benchmark the implementations on synthetic Gaussian data with $K = 5$ clusters and sample sizes scaling up to $n = 100,000$.

## Performance & Memory Scaling

| Sample Size ($n$) | `cluster::silhouette()` Time (s) | `Silhouette()` Time (s) | `cluster::silhouette()` Memory | `Silhouette()` Memory |
|:---:|:---:|:---:|:---:|:---:|
...
*Note: `OOM` indicates that the execution failed due to memory exhaustion (Out of Memory).*

As shown in the benchmark results, `cluster::silhouette()` fails to compute when $n \ge 25,000$ due to the quadratic memory scaling ($O(n^2)$). The `Silhouette` package, running at $O(nK)$, processes $n=100,000$ observations in **less than 0.02 seconds** using only **4.0 MB of memory**, resolving the computational bottleneck entirely.
